...
def download_data(metadata, dataset,
                  video_ids,
                  dataset_splits,
                  download_dir,
                  keep_zip,
    ...
    videos_sorted = sorted(set(video_ids))
    for vidx in tqdm(range(len(videos_sorted))):
        video_id = videos_sorted[vidx]
...
if __name__ == "__main__":
    ...
    # df = get_metadata(dataset, download_dir)
    # if None is df:
    #     print(f"Error retrieving metadata for dataset {dataset}")
    df = pd.read_csv(join(download_dir, dataset, "metadata.csv"))
    ...
    batch_size = int(len(video_ids_) / batch_size) + 1
    if len(video_ids_) > batch_size:
        if args.idx == 0:
            selected_scenarios = video_ids_[:batch_size]
            selected_splits = splits_[:batch_size]
        else:
            selected_scenarios = video_ids_[args.idx * batch_size: min(len(video_ids_), (args.idx + 1) * batch_size)]
            selected_splits = splits_[args.idx * batch_size: min(len(video_ids_), (args.idx + 1) * batch_size)]
    else:
        selected_scenarios = video_ids_
        selected_splits = splits_

    download_data(df, dataset,
                  selected_scenarios,
                  selected_splits,
                  args.download_dir,
                  args.keep_zip,
