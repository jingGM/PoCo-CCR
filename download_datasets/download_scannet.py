...
def main():
    ...
    sorted(release_scans)

    batch_size = int(len(release_scans) / args.batch_size) + 1
    if len(release_scans) > batch_size:
        if args.idx == 0:
            release_scans = release_scans[:batch_size]
        else:
            release_scans = release_scans[args.idx * batch_size: min(len(release_scans), (args.idx + 1) * batch_size)]
    else:
